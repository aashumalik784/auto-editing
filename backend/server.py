...
async def upload_video(video: UploadFile = File(...)):
    try:
        ...
        print(f"Video uploaded: {video_id}")        return {"success": True, "videoId": video_id, "message": "Video uploaded successfully"}
...
@app.get("/api/download/{video_id}")async def download_video(video_id: str):
    output_path = os.path.join(PROCESSED_DIR, f"{video_id}_edited.mp4")
    if not os.path.exists(output_path):
        raise HTTPException(status_code=404, detail="Video not found")
