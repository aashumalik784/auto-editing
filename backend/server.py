...
async def root():
    return {
        "status": "healthy",        "service": "Auto-Editing API",
        "version": "1.0.0"
    }
...
async def process_video(request: ProcessRequest, background_tasks: BackgroundTasks):
    try:
        ...
        return {
            "success": True,
            "jobId": job_id,            "message": "Processing started"
        }
...
async def download_video(video_id: str):    output_path = os.path.join(PROCESSED_DIR, f"{video_id}_edited.mp4")
