...
async def get_video(video_id: str):
    ...
    except Exception as e:        print(f"Get video error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def process_video_task(job_id: str, video_id: str, options: Dict):
    try:
        input_path = None        for ext in ['.mp4', '.avi', '.mov', '.mkv']:
            test_path = os.path.join(UPLOAD_DIR, f"{video_id}{ext}")
            if os.path.exists(test_path):
        ...
        processing_jobs[job_id]["status"] = "failed"        processing_jobs[job_id]["message"] = f"Processing failed: {str(e)}"
